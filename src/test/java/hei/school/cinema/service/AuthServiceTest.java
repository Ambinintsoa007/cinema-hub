...
class AuthServiceTest {
  ...
  void register_should_reject_short_password() {
    RegisterRequest request =
        new RegisterRequest(
            "John", "Doe", LocalDate.of(2000, 1, 1), "john@example.com", "+261340000000", "short");
  ...
  void authenticate_should_return_user_when_credentials_are_valid() {
    ...
    UserEntity entity = userEntity(passwordEncoder.encode(password), UserStatusEntity.ACTIVE);

    when(userRepository.findByEmailIgnoreCase("john@example.com")).thenReturn(Optional.of(entity));
  ...
  void authenticate_should_reject_unknown_email() {
    ...
    ApiException exception =
        assertThrows(ApiException.class, () -> authService.authenticate(request));
  ...
  void authenticate_should_reject_wrong_password() {
    UserEntity entity =
        userEntity(passwordEncoder.encode("correct-password"), UserStatusEntity.ACTIVE);
  ...
  void authenticate_should_reject_disabled_user() {
    UserEntity entity =
        userEntity(passwordEncoder.encode("password123"), UserStatusEntity.DISABLED);
  ...
  private UserEntity userEntity(String passwordHash, UserStatusEntity status) {
